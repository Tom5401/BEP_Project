...
with open(directory+'gains_follow_car2.txt','w') as f:
    for line in gains:
        f.write(line)
        f.write('\n')
...
if __name__ == '__main__':
    try:
        rospy.init_node('follow_car2', anonymous=False)
        rate = rospy.Rate(10)
        rospy.on_shutdown(stopJetracer)
        pub_steering = rospy.Publisher('steering3',Float32,queue_size=8)
        pub_throttle = rospy.Publisher('throttle3',Float32,queue_size=8)
        rospy.Subscriber('Optitrack_data_topic_3', PoseStamped, platoonCallback, queue_size=1,  buff_size=2**24)
