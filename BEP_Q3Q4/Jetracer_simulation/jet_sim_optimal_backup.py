...
class Car:
    def __init__(self, x, y, angle=np.pi/2, length=0.2, max_steering=radians(20), max_acceleration=5.0):
        ...
        self.u = np.linspace(0,2*np.pi,500)
        ...
        self.K2 = 10
        C = self.C()
        self.d_e_prev,self.i_prev = spatial.KDTree(self.path).query([C.x,C.y])
    ...
    def steeringController(self):
        ...
        path_seg = self.slicer(self.path,self.i_prev-5,self.i_prev+5)
        print('segment index:',self.i_prev-5,self.i_prev+5)
        d_abs,i = spatial.KDTree(path_seg).query([C.x,C.y])
        i += self.i_prev - 5
        #print('i,i_prev:',i,self.i_prev)
        self.i_prev = i
        if i < len(self.u)-1:
            point_vector = self.path[i-1] - self.path[i+1]
        else:
            point_vector = self.path[i-1] - self.path[0]
        theta_d = np.arctan2(point_vector[1],point_vector[0])
        theta_e = self.angle - theta_d
        if theta_e > np.pi:
            theta_e = theta_e - 2*np.pi
        elif theta_e < -np.pi:
            theta_e = theta_e + 2*np.pi
        d_e = (C.y-self.path[i][1])*np.cos(theta_d) - (C.x-self.path[i][0])*np.sin(theta_d)
        print(d_e, theta_e)
